package thecompletereferenc.forkjoin;

//A simple example that uses RecursiveTask<V>


import java.util.concurrent.ForkJoinPool;
import java.util.concurrent.RecursiveTask;

public class RecursiveTaskDemo {
    public static void main(String[] args) {
        //Create a task pool.
        ForkJoinPool fjp = new ForkJoinPool();

        double[] nums = new double[5000];
        //Initialize nums with values that alternate between
        //positive and negative.
        for (int i = 0; i < nums.length; i++) {
            nums[i] = (double) (((i % 2) == 0) ? i : -i);
        }

        SumRec task = new SumRec(nums, 0, nums.length);
        //Start the ForkJoinTasks. Notice that, in this case,
        //invoke() returns result.
        double summation = fjp.invoke(task);

        System.out.println("Summation " + summation);
    }
}

//A RecursiveTask that computes the summation of an array of doubles.
class SumRec extends RecursiveTask<Double> {
    //The sequential threshold value.
    final int seqThresHold = 500;

    //Array to be accessed.
    double[] data;

    //Determines what part of data to process.
    int start, end;

    SumRec(double[] vals, int s, int e) {
        data = vals;
        start = s;
        end = e;
    }

    //Find the summation of an array of doubles.
    @Override
    protected Double compute() {
        double sum = 0;
        if((end - start) < seqThresHold) {
            //Sum the elements.
            for (int i = start; i < end; i++) {
                sum += data[i];
            }
        } else {
            // Otherwise, continue to break the data into smaller pieces.
            //Find the midpoint
            int middle = (start + end)/2;

            //Invoke new tasks, using the subdivided data.
            SumRec subTaskA = new SumRec(data, start, middle);
            SumRec subTaskB = new SumRec(data, middle, end);
            //Start each subtask by forking.
            subTaskA.fork();
            subTaskB.fork();

            //Wait for the subtasks to return, and aggregate the results.
            sum = subTaskA.join() + subTaskB.join();

            /*
            方式1
            subTaskA.fork();
            sum = subTaskB.invoke() + subTaskA.join();

            方式2
            sum = subTaskB.compute() + subTaskA.join();
             */
        }

        return sum;
    }
}

















