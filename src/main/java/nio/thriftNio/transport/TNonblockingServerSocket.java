...
package nio.thriftNio.transport;
...
public class TNonblockingServerSocket extends TNonblockingServerTransport {
   ...
   */
  public TNonblockingServerSocket(int port) throws Exception {
    this(port, 0);
  }
   ...
   * Creates just a port listening server socket
   */
  public TNonblockingServerSocket(int port, int clientTimeout) throws Exception {
    this(new NonblockingAbstractServerSocketArgs().port(port).clientTimeout(clientTimeout));
  }

  public TNonblockingServerSocket(InetSocketAddress bindAddr) throws Exception {
    this(bindAddr, 0);
  }

  public TNonblockingServerSocket(InetSocketAddress bindAddr, int clientTimeout) throws Exception {
    this(new NonblockingAbstractServerSocketArgs().bindAddr(bindAddr).clientTimeout(clientTimeout));
  }

  public TNonblockingServerSocket(NonblockingAbstractServerSocketArgs args) throws Exception {
    ...
    try {
      ...
      serverSocket_ = null;
      throw new Exception("Could not create ServerSocket on address " + args.bindAddr.toString() + ".");
    }
  }
  ...
  protected TNonblockingSocket acceptImpl() throws Exception {
    if (serverSocket_ == null) {
